...
public class CoolShapeEntity extends ChargeableSpellEntity {
    ...
    private void clientTick() {
        ...
        for (int i = 0; i < n; i++) {
            ...
            level().addParticle(getSpell().colorParticle(new MAParticleType(ParticleUtils.getParticleType(affinities[random.nextInt(affinities.length)])), getCaster()), pos.x, pos.y, pos.z, 0, random.nextDouble(0.1), 0);
        }
                        ...
                        for (var aabb : shape.toAabbs()) {
                            for (var dir : Direction.values()) {
                                ...
                                if (aabb.contains(h.add(-0.5, -0.5, -0.5).scale(1.1).add(0.5, 0.5, 0.5)))
                                    continue;
                                level().addParticle(getSpell().colorParticle(new MAParticleType(ParticleUtils.getParticleType(affinities[random.nextInt(affinities.length)])), getCaster()), p.x, p.y, p.z, 0, 0, 0);
                            }
                        }
