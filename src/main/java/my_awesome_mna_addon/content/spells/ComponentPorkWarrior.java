package my_awesome_mna_addon.content.spells;
...
public class ComponentPorkWarrior extends SpellEffect {
	...
	static {
		required_reagents.add(new SpellReagent(PhantomComponent.instance,new ItemStack(Items.COOKED_PORKCHOP), false, false, true));
	}

	public ComponentPorkWarrior(ResourceLocation registryName, ResourceLocation guiIcon) {
		super(registryName, new AttributeValuePair(Attribute.DURATION, 30, 10, 300, 10, 0.5f));
	}
	...
	public ComponentApplicationResult ApplyEffect(SpellSource source, SpellTarget target, IModifiedSpellPart<SpellEffect> attributes, SpellContext context) {
		...
		Mob spawn = (Mob)type.create(context.getLevel()); //double check that all the EntityTypes inherit from Mob; these all do.
		if (spawn == null) {
			return ComponentApplicationResult.FAIL;
		}
